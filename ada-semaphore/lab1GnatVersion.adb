...
--        F = MB*MC+MO*MT-e*MZ               --
-----------------------------------------------
with Ada.Text_IO, Ada.Integer_Text_IO, System.Multiprocessors, Ada.Synchronous_Task_Control;
...
procedure Lab1 is
	...
	function sub(MA,MB: in Matrix; b,e: in integer; MC: in out Matrix) return Matrix is
	begin
		for i in b..e loop
    				for j in 1..n loop
    						MC(i)(j):=MA(i)(j)-MB(i)(j);
	...
	function f(MA: in out Matrix; MB,MC,MO,MT,MZ: in Matrix; k,b,e:integer) return Matrix is
	begin
    		MA:=sub(add(mul(MB,MC,b,e),mul(MO,MT,b,e),b,e),mul(MZ,k,b,e),b,e,MA);	
		return MA;
	end f;
	...
	procedure start_tasks is
		...
		task T1 is
    		...
    		begin
    			...
    			--account
    			MA:=f(MA,MB,MC1,MO,MT1,MZ,e,1,h);
